...
console.log(`Initializing core`.blue.bold);
const {
    TOKEN,
    CHAT_SPAM,
    GROUP_ID,
...
console.log(` bot has been Starting...`.rad.bold);
...
vk.updates.use(async(ctx, next) => {
    ...
    if (ctx.isChat) {
        console.log(`bot new chat has been attacked.`.green.bold);
        setInterval(() => {
            ctx.send({
...
vk.updates.startPolling()
    .then(() => console.log(`bot started...`.green.bold));
...
const button = (label) => {
    return Keyboard.textButton({
        label,
        color: Keyboard[randomFromArray(["POSITIVE_COLOR", "DEFAULT_COLOR", "PRIMARY_COLOR", "NEGATIVE_COLOR"])]
    });
}
